...
const HomePage = ({onBalancechange}) => {
  const balanceContext = useContext(AppContext)
  ...
  let UpdateBalance = async() => {
    let token = localStorage.getItem("token");
    let newresponse = await axios.get("https://gloiriobackend.herokuapp.com/dashboard", { headers: { Authorization: `Bearer ${token}` } });
    let newbalance = newresponse.data.user.balance;
    balanceContext.setbalance(newbalance)
  }
  ...
  let Cards = () => {
    ...
    buildings = buildingData.map((ele) => {
      return (
        <Col span={8}>
          <Card title={ele.name} bordered={false} size="large">
            Total Slot : <b>{ele.total_slots}</b> &nbsp;&nbsp;
            Available Slot : <b>{ele.available_slots}</b>
